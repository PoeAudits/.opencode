---
description: Resumes work from a previous handoff session which are stored in `.opencode/handoffs`.
---

Resumes work from a previous handoff session which are stored in `.opencode/handoffs`.
...
echo ""
for file in .opencode/handoffs/*.md; do
  if [ -f "$file" ]; then
